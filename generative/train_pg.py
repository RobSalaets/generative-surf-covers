...
from generative.config import config
import generative.network as net
from generative.dataloader import MatlabDataLoader
...
class Trainer:
    def __init__(self, config):
        ...
        self.resl = 2
        self.lr = config.lr
        self.eps_drift = config.eps_drift #TODO
        self.smoothing = config.smoothing # TODO?
        ...
        self.phase = 'stab'
        self.countdown = self.TICK * (self.trns_tick + self.stab_tick)
    ...
    def resolution_scheduler(self):
        self.countdown = max(0, self.countdown - self.loader.batchsize)
        if self.countdown >= self.trns_tick*self.TICK:
            if self.fadein['gen'] is not None:
                dalpha = (self.trns_tick+self.stab_tick - self.countdown/self.TICK) / self.trns_tick / self.loader.batchsize
                self.fadein['gen'].update_alpha(dalpha)
                self.fadein['dis'].update_alpha(dalpha)
        else:
            self.phase = 'stab'
        ...
        if (self.kimgs % self.TICK) < (prev_kimgs % self.TICK):
            ...
            # grow network.
            if self.countdown == 0.0 and self.resl < self.max_resl:
                self.lr = self.lr * float(self.config.lr_decay)
                self.resl += 1
                self.G.module.grow_network(self.resl)
                self.D.module.grow_network(self.resl)
                self.renew_everything()
                self.countdown = self.TICK * (self.trns_tick+self.stab_tick)
                self.phase = 'trans'
    ...
    def resl_scheduler(self):
        ...
        # updata fadinlayer, update phase, update 'complete'
        if self.fadein['gen'] is not None:
    ...
    def train(self):
        ...
        for step in range(2, self.max_resl+1+5):
            for it in tqdm(range(0,(self.trns_tick+self.stab_tick)*self.TICK, self.loader.batchsize)):
                raw = self.loader.get_batch(self.use_cuda)
                self.x.data = self.feed_interpolated_input(raw)
                self.stack = self.stack + len(raw)
                ...
                # logging.
                log_msg = ' [E:{0}][T:{1}][{2:6}/{3:6}]  errD: {4:.4f} | errG: {5:.4f} | [lr:{9:.5f}][cur:{6:.3f}][resl:{7:4}][{8}]'.format(self.epoch, self.globalTick, self.stack, len(self.loader.dataset), loss_d.item(), loss_g.item(), self.resl, int(pow(2,floor(self.resl))), self.phase, self.lr)
                tqdm.write(log_msg)
...
if __name__ == '__main__':
    ...
    trainer.train()
    # import cProfile
    # cProfile.run('trainer.train()', 'stats_pg3')
