...
class Generator(nn.Module):
    def __init__(self, nz, img_size, nc, hg):
        ...
        self.c4 = nn.ConvTranspose2d(hg // 4, hg // 8, 3, 2, 1, 1, bias=False)
        self.bn4 = nn.BatchNorm2d(hg // 8)


        # 16x16
        self.c5 = nn.ConvTranspose2d(hg // 8, nc, 3, 2, 1, 1, bias=False)
...
class Discriminator(nn.Module):
    def __init__(self, img_size, nc, hd):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(nc, hd //8, 3, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf) x 32 x 32
            nn.Conv2d(hd // 8, hd//4, 3, 2, 1, bias=False),
            nn.BatchNorm2d(hd//4),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if iters % 1000 == 0:
                ...
                with torch.no_grad():
                    ...
                    plt.imshow(
                        np.transpose(vutils.make_grid(fake[0], padding=2, normalize=True).cpu(), (1, 2, 0)))
                plt.show()
                netG.train()
            iters += 1
